...
HTML_EXTENSIONS = ['.htm', '.html', '.htmls']
...
class MainWindow(QMainWindow):
    ...
    def file_open(self):
        path, _ = QFileDialog.getOpenFileName(self, "Open file", "", "HTMLS documents (*.htmls);TextS documents (*.txts);All files (*.*)")
        try:
            with open(path, 'rb') as f:
                data = f.read()
            pack_plaint = Authentication.decrypt(data)
            flag, docx_data, sig_name = Authentication.hash_separate(pack_plaint, 256)
            if flag:
                if sig_name == username:
                    text = docx_data.decode()
                else:
                    text = docx_data.decode()
                    self.dialog_critical("Signature of this document is invalid !\n" + "The last author name: {}".format(sig_name))
            else:
                self.dialog_critical("Signature of this document is invalid !\n Document is edited by unknown author !")
    ...
    def file_save(self):
        ...
        try:
            ...
            data = text.encode()
            pack, sig_size = Authentication.hash_concatenate(data)
            encrypted_pack = Authentication.encrypt(pack)
            with open(self.path, 'wb') as f:
                f.write(encrypted_pack)

        except Exception as e:
            self.dialog_critical(str(e))

    def file_saveas(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save file", "", "HTMLS documents (*.htmls);TextS documents (*.txts);All files (*.*)")
    ...
    def update_title(self, author_name=None):
        if author_name:
            self.setWindowTitle("%s - Safe Notepad - Last Author: %s" % ((os.path.basename(self.path) if self.path else "Untitled"), author_name))
        else:
            self.setWindowTitle("%s - Safe Notepad" % (os.path.basename(self.path) if self.path else "Untitled"))
...
if __name__ == '__main__':
    ...
    app.setApplicationName("Safe Notepad")
